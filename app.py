...
def faculty_login():
    ...
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT * FROM faculty_login WHERE faculty_id=%s AND password=%s", (faculty_id, password))
...
def student_details():
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT roll_number, name, department, semester, email FROM students")
...
def add_student_result():
    if request.method == 'POST':
        ...
        try:
            cursor = db.cursor()
            query = """
            INSERT INTO student_results (roll_number, python_marks, web_dev_marks, chm_marks, english_marks, iot_marks)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
